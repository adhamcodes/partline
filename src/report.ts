...
export function report(artifact: Artifact): string {
  ...
  const lines = [
    ...
    ...artifact.comparison.map(row => `| ${plain(artifact.job.targets.find(t => t.id === row.targetId)!.name)} | ${row.eligibility} | ${row.totalMinor !== undefined ? `${artifact.job.currency} ${(row.totalMinor / 100).toFixed(2)}` : 'unknown'} | ${row.readyAt ?? 'unknown'} | ${Math.round(row.coverage * 100)}% |`), '',
