# Proposed next live experiment — not authorization
...
- The human approves the exact request, destination, questions, one-call budget and disclosure. Wire the live transport and adapter only after that explicit authorization in a later mission. Mission 001 has no live CLI command.
...
Abort further experiments if disclosure, refusal behavior, call count or provenance fails. A one-call consent does not authorize another run. This experiment validates the integration; a subsequent separately approved small supplier pilot is needed to validate product usefulness.
